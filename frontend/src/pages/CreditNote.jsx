                    ...
                    <div className="card border-0 shadow-sm credit-note-create">
                        <div className="card-body">
                            <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                                <div>
                                    <h5 className="fw-bold mb-1">{view === "edit" ? "Edit Credit Note" : "Create Credit Note"}</h5>
                                    <div className="text-muted small">
                                        {view === "edit"
                                            ? "Edit all financial details below. The original invoice link remains fixed for audit safety."
                                            : "Select an invoice, choose what to credit, review the automatic settlement, then post."}
                                    </div>
                                </div>
                                <div className="d-flex flex-wrap gap-2 align-items-center">
                                    {view === "edit" && (
                                        <input
                                            ...
                                            maxLength={16}
                                            onChange={(e) => setForm((f) => ({ ...f, creditNoteNumber: e.target.value.toUpperCase() }))}
                                            placeholder="Credit Note Number"
                                        />
                                    )}
                                    <select className="form-select form-select-sm" value={form.creditMode} onChange={(e) => setForm((f) => ({ ...f, creditMode: e.target.value }))}>
                                        <option value="ITEM">Item Based</option>
                                        <option value="AMOUNT">Amount Based</option>
                                    </select>
                                    {form.reason === "Sales Return" && (
                                        <div className="form-check form-switch border rounded px-4 py-2 mb-0">
                                            <input className="form-check-input" type="checkbox" checked={!!form.stockAffecting} onChange={(e) => setForm((f) => ({ ...f, stockAffecting: e.target.checked }))} id="stockAffecting" />
                                            <label className="form-check-label" htmlFor="stockAffecting">Return Stock</label>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="row g-3 mb-3">
                                <div className="col-md-3">
                                    <label className="form-label fw-semibold">Credit Note Date</label>
                                    <input type="date" className="form-control" value={form.creditNoteDate} onChange={(e) => setForm((f) => ({ ...f, creditNoteDate: e.target.value }))} />
                                </div>
                                <div className="col-md-3">
                                ...
                                <div className="col-md-6 position-relative">
                                    ...
                                    {view === "edit" ? (
                                        <div className="form-control bg-light">
                                            <strong>{selectedOrder?.orderNumber || invoiceSearch || "-"}</strong>
                                            <span className="text-muted ms-2">Original invoice cannot be changed after posting.</span>
                                        </div>
                                    ) : (
                                        <>
                                            <input className="form-control" value={invoiceSearch} placeholder="Search Invoice No., Client or GSTIN" onFocus={() => setSearchResultsOpen(Boolean(invoiceSearch))} onChange={(e) => searchInvoices(e.target.value)} />
                                            {searchResultsOpen && invoiceSearch.trim() && (
                                                <div className="position-absolute bg-white border rounded shadow-sm w-100 invoice-search-menu">
                                                    {orderLoading
                                                        ? <div className="p-3 text-muted">Searching invoices...</div>
                                                        : orders.length === 0
                                                            ? <div className="p-3 text-muted">No matching invoices.</div>
                                                            : orders.map((order) => (
                                                                <button type="button" key={order._id} className="dropdown-item p-3 border-bottom text-start" onClick={() => loadOrder(order._id)}>
                                                                    <div className="fw-semibold">{order.orderNumber}</div>
                                                                    <div className="small">{order.companyName || "-"} {order.gstNumber ? `• ${order.gstNumber}` : ""}</div>
                                                                    <div className="small text-muted">{formatDate(order.orderDate)} • {money(order.roundOffFinalRevenue)}</div>
                                                                </button>
                                                            ))}
                                                </div>
                                            )}
                                ...
                                <>
                                    <div className="card border mb-3">
                                        <div className="card-body">
                                            <div className="row g-3">
                                                ...
                                                <div className="col-lg-3"><div className="small text-muted">Invoice Total</div><div className="fw-semibold">{money(selectedOrder.invoiceTotal ?? selectedOrder.roundOffFinalRevenue)}</div></div>
                                                <div className="col-lg-3"><div className="small text-muted">Paid</div><div>{money(selectedOrder.paidAmount)}</div></div>
                                            ...
                                            <div className="card-body">
                                                ...
                                                <div className="row g-3">
                                                    <div className="col-md-6">
                                                        ...
                                                        <input type="number" min="0" step="0.01" className="form-control" value={form.manualTaxableAmount} onChange={(e) => setForm((f) => ({ ...f, manualTaxableAmount: r2(e.target.value) }))} />
                                                    </div>
                                                    <div className="col-md-6">
                                                        <label className="form-label">Tax Rate %</label>
                                                        <input type="number" min="0" max="100" step="0.01" className="form-control" value={form.manualTaxRate} onChange={(e) => setForm((f) => ({ ...f, manualTaxRate: r2(e.target.value) }))} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {form.creditMode === "ITEM" && (
                                        <div className="d-flex flex-wrap gap-2 mb-3 no-print">
                                            <button className="btn btn-outline-primary btn-sm" onClick={applyAllAvailable}>Credit All Available</button>
                                            <button className="btn btn-outline-secondary btn-sm" onClick={clearAllItemCredits}>Clear Quantities</button>
                                        </div>
                                    )}
                                            ...
                                            <div className="card border h-100">
                                                <div className="card-body">
                                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                                        <div>
                                                            <h6 className="fw-bold mb-1">Settlement</h6>
                                                            <div className="small text-muted">{form.autoSettlement ? "Automatic: apply to invoice due first, then refund the remaining amount." : "Manual: enter the exact allocation of this Credit Note."}</div>
                                                        </div>
                                                        <div className="form-check form-switch">
                                                            <input className="form-check-input" type="checkbox" id="autoSettlement" checked={!!form.autoSettlement} onChange={(e) => setForm((f) => ({ ...f, autoSettlement: e.target.checked }))} />
                                                            <label className="form-check-label" htmlFor="autoSettlement">Automatic</label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="row g-3">
                                                        <div className="col-md-4">
                                                            ...
                                                            <select className="form-select" value={form.adjustmentType} disabled>
                                                                <option>Outstanding</option>
                                                            </select>
                                                        </div>
                                                        <div className="col-md-4">
                                                            <label className="form-label">Adjust Against Invoice Due</label>
                                                            <input type="number" className="form-control" min="0" max={invoiceDueAvailableForAdjustment} step="0.01" value={form.adjustmentAmount} readOnly={form.autoSettlement} onChange={(e) => setForm((f) => ({ ...f, adjustmentAmount: r2(e.target.value) }))} />
                                                            <div className="small text-muted mt-1">Maximum: {money(invoiceDueAvailableForAdjustment)}</div>
                                                        </div>
                                                        <div className="col-md-4">
                                                            <label className="form-label">Refund Amount</label>
                                                            <input type="number" className="form-control" min="0" max={totals.grandTotal} step="0.01" value={form.refundAmount} readOnly={form.autoSettlement} onChange={(e) => setForm((f) => ({ ...f, refundAmount: r2(e.target.value) }))} />
                                                        </div>
                                                        <div className="col-md-4">
                                                            <label className="form-label">Refund Method</label>
                                                            <select className="form-select" value={form.refundMethod} onChange={(e) => setForm((f) => ({ ...f, refundMethod: e.target.value }))}>
                                                                {REFUND_METHODS.map((method) => <option key={method}>{method}</option>)}
                                                            </select>
                                                        </div>
                                                        <div className="col-md-4">
                                                            <label className="form-label">Customer Credit</label>
                                                            <input type="number" className="form-control" min="0" max={totals.grandTotal} step="0.01" value={form.customerCreditAmount} readOnly={form.autoSettlement} onChange={(e) => setForm((f) => ({ ...f, customerCreditAmount: r2(e.target.value) }))} />
                                                        </div>
                                                        <div className="col-md-4">
                                                            <label className="form-label">Note</label>
                                                            <textarea className="form-control" rows="2" maxLength="1000" value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="col-lg-5">
                                            <div className="card border h-100">
                                                <div className="card-body">
                                                    ...
                                                    <div className="d-flex justify-content-between"><span>Discount</span><span>{money(totals.discount)}</span></div>
                                                    <div className="d-flex justify-content-between"><span>Taxable</span><span>{money(totals.taxable)}</span></div>
                                                    <div className="d-flex justify-content-between"><span>Tax</span><span>{money(totals.tax)}</span></div>
                                                    <div className="d-flex justify-content-between"><span>Round Off</span><span>{money(totals.roundOff)}</span></div>
                                                    <hr />
                                                    <div className="d-flex justify-content-between fs-5 fw-bold"><span>Credit Note Total</span><span>{money(totals.grandTotal)}</span></div>
                                                    <div className="d-flex justify-content-between mt-2"><span>Invoice Adjustment</span><span>{money(form.adjustmentAmount)}</span></div>
                                                    <div className="d-flex justify-content-between"><span>Refund</span><span>{money(form.refundAmount)}</span></div>
                                                    <div className="d-flex justify-content-between"><span>Customer Credit</span><span>{money(form.customerCreditAmount)}</span></div>
                                                    <div className={`small mt-2 ${r2(n(form.adjustmentAmount) + n(form.refundAmount) + n(form.customerCreditAmount)) === r2(totals.grandTotal) ? "text-success" : "text-danger"}`}>
                                                        Allocated: {money(r2(n(form.adjustmentAmount) + n(form.refundAmount) + n(form.customerCreditAmount)))} / {money(totals.grandTotal)}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="d-flex justify-content-end gap-2 mt-3 no-print">
                                <button className="btn btn-outline-secondary" onClick={() => { setView("list"); setEditingNote(null); }}>Cancel</button>
                                <button className="btn btn-primary" disabled={saving || !selectedOrder} onClick={view === "edit" ? updateNote : save}>
                                    {saving ? <><span className="spinner-border spinner-border-sm me-2" />Saving...</> : (view === "edit" ? "Update Credit Note" : "Post Credit Note")}
                                </button>
                            </div>
                        </div>
                    </div>
