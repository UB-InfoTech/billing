                            ...
                            {selectedOrder && <>
                                ...
                                <div className="row g-3">
                                    <div className="col-lg-7"><div className="card border h-100"><div className="card-body"><h6 className="fw-bold">Settlement</h6><div className="row g-3">
                                        <div className="col-md-4"><label className="form-label">Adjustment Type</label><select className="form-select" value={form.adjustmentType} onChange={(e) => setForm((f) => ({ ...f, adjustmentType: e.target.value }))}><option>Outstanding</option><option>Advance</option><option>Other</option></select></div><div className="col-md-4"><label className="form-label">Adjust Against Invoice Due</label><input type="number" className="form-control" min="0" max={invoiceDueAvailableForAdjustment} step="0.01" value={form.adjustmentAmount} onChange={(e) => setForm((f) => ({ ...f, adjustmentAmount: r2(e.target.value) }))} /><div className="small text-muted mt-1">Maximum: {money(invoiceDueAvailableForAdjustment)}</div></div>
                                        <div className="col-md-4"><label className="form-label">Refund Amount</label><input type="number" className="form-control" min="0" max={totals.grandTotal} step="0.01" value={form.refundAmount} onChange={(e) => setForm((f) => ({ ...f, refundAmount: r2(e.target.value) }))} /></div>
                                        ...
                                        <div className="col-md-4"><label className="form-label">Customer Credit</label><input type="number" className="form-control" min="0" max={totals.grandTotal} step="0.01" value={form.customerCreditAmount} onChange={(e) => setForm((f) => ({ ...f, customerCreditAmount: r2(e.target.value) }))} /></div><div className="col-md-4"><label className="form-label">Note</label><textarea className="form-control" rows="2" maxLength="1000" value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} /></div>
                                    </div></div></div></div>
